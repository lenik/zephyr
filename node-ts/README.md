...
`zephyr` is a Node.js template for small command-line apps, using Meson for build/test/install.
`puff1` is one **example app** in this template; more apps can be added in the same repository.

## Repository layout

- `src/` - Node.js source files (`*.mjs`)
- `tests/` - JavaScript smoke/unit tests
...
```bash
sudo apt install meson ninja-build nodejs gettext
```
...
Meson runs a Node-based smoke/unit test from `tests/test_puff1.mjs`.
