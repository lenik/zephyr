...
`zephyr` is a Java + Meson project template for small command-line apps.
...
```bash
sudo apt install meson ninja-build default-jdk gettext
```
...
Meson runs a Java smoke/unit test (`TestPuffLib`) in `meson test`.

## i18n (gettext)

`puff1` uses gettext catalogs under `po/` (`*.po` + generated `.mo` files).
