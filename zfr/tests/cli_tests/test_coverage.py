...
from support import REPO, ROOT, TOOLS, ZEPHYR, _env, run_zephyr, run_wrapper, add_src_to_path
...
class ZephyrDispatcherTests(unittest.TestCase):
    ...
    def test_cli_version_matches_runtime(self) -> None:
        ...
        from lib import cli_version, format_cli_version_banner
    ...
    def test_every_wrapper_help(self) -> None:
        for name in WRAPPERS:
            with self.subTest(name=name):
                proc = run_wrapper(name, "--help", check=False)
                self.assertEqual(proc.returncode, 0, proc.stderr)
                self.assertTrue(proc.stdout.strip())
