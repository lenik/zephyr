...
from support import ROOT, REPO, TOOLS, ZEPHYR, _env, run_zephyr, run_wrapper, add_src_to_path
...
class ZephyrHelpTests(unittest.TestCase):
    ...
    def test_ize_wrapper_help(self) -> None:
        proc = run_wrapper("zfr-ize", "--help", check=False)
...
class ZephyrDetectTests(unittest.TestCase):
    ...
    def test_release_wrapper_help(self) -> None:
        proc = run_wrapper("zfr-release", "--help", check=False)
        self.assertEqual(proc.returncode, 0, proc.stderr)
    ...
    def test_release_options_implications(self) -> None:
        ...
        from release import add_release_arguments, namespace_to_options
...
class ZephyrCreateProjectTests(unittest.TestCase):
    ...
    def test_packaging_under_zfr_only(self) -> None:
        ...
        self.assertTrue((REPO / "README-zh_CN.md").is_file())
        self.assertFalse((ROOT / "README.md").exists())
        self.assertFalse((ROOT / "README-zh_CN.md").exists())
        self.assertFalse((REPO / "README-zh.md").exists())
