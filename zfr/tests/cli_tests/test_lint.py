...
class LintStyleInfoDefaultsTests(unittest.TestCase):
    def test_non_interactive_defaults_info_on(self) -> None:
        from terminal import default_for_ai_purpose, default_lint_style_info
...
class AiHintReportTests(unittest.TestCase):
    def test_ai_hint_when_pass_and_dirty(self) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix="zfr-ai-hint-") as tmp:
            ...
            with patch("lint.report._git_working_tree_dirty", return_value=True):
                text = format_report(
                    root,
                    "demo",
                    "c",
    ...
    def test_no_ai_hint_when_clean(self) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix="zfr-ai-clean-") as tmp:
            root = Path(tmp)
            with patch("lint.report._git_working_tree_dirty", return_value=False):
                text = format_report(
                    root,
                    "demo",
                    "c",
...
class ZephyrLintSourceTests(unittest.TestCase):
    def test_skips_example_commons_module(self) -> None:
        ...
        from lint.util import is_example_shared_src
        ...
        with tempfile.TemporaryDirectory(prefix="zfr-lint-src-") as tmp:
            ...
            # Example commons skipped — only the clean ok for source.long/size.
            self.assertTrue(
                any(f.code == "source.long" and f.severity == "ok" for f in findings)
            )
            self.assertFalse(
                any(f.code == "source.long" and f.severity != "ok" for f in findings)
            )
    ...
    def test_template_coverage_skips_commons_and_renames_spec(self) -> None:
        from lint.template import check_template_gaps, _expected_rel, is_optional_scaffold
        from lint.util import is_example_shared_rel
    ...
    def test_medium_source_emits_lint_note(self) -> None:
        from lint.source_size import check_source_size, _extract_subdir_fix
    ...
    def test_zfr_package_has_no_source_size_notes(self) -> None:
        from lint import collect_findings
...
class SeverityRemapTests(unittest.TestCase):
    ...
    def test_cli_parse_levels_and_strict_alias(self) -> None:
        ...
        from lint import add_arguments
