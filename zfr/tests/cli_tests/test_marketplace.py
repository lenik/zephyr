...
from release.marketplace import ensure_pnpm_cli  # noqa: E402
...
class EnsurePnpmCliTests(unittest.TestCase):
    def test_uses_path_when_present(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            cwd = Path(td)
            with patch("release.marketplace.shutil.which", return_value="/usr/bin/ovsx"):
                self.assertEqual(
    ...
    def test_uses_pnpm_exec_when_local(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            with patch("release.marketplace.shutil.which", side_effect=which):
                with patch(
                    "release.marketplace._pnpm_exec_has",
                    return_value=True,
                ):
                    self.assertEqual(
                        ensure_pnpm_cli("ovsx", "ovsx", cwd=cwd),
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            with patch("release.marketplace.shutil.which", side_effect=which):
                with patch(
                    ...
                    with patch(
                        "release.marketplace.subprocess.run",
                        side_effect=run,
                    ):
                        with patch("release.marketplace.log1"):
