...
def _command_uses_translate_sync(command: str) -> bool:
    c = command.replace("\\", "/")
    return "translate" in c and "--sync" in c


def _command_uses_build_look(command: str) -> bool:
    c = command.replace("\\", "/")
...
def check_posync(root: Path, role: str) -> list[Finding]:
    ...
    if "posync" not in blocks:
        return [
            Finding(
                ...
                "meson.build",
                fix=_(
                    "Add run_target posync calling `zfr translate --sync` "
                    "(or scripts/posync.sh). Or run `zfr ize`."
                ),
            )
        ]
    body = blocks["posync"]
    if _command_uses_translate_sync(body):
        return [
            Finding(
                "ok",
                "layout.posync",
                _("posync uses zfr translate --sync"),
                "meson.build",
            )
        ]
    ...
    if script.is_file() and not _command_uses_scripts(body, "posync"):
        return [
            Finding(
                ...
                "meson.build",
                fix=_(
                    "Point run_target('posync') at `zfr translate --sync` "
                    "or scripts/posync.sh. Or run `zfr ize`."
                ),
            )
        ]
    return [
        Finding(
            "warn",
            "layout.posync",
            _("posync is inline in meson.build; use zfr translate --sync"),
            "meson.build",
            fix=_(
                "Replace the inline posync body with `zfr translate --sync` "
                "(Python import inside zfr). Or run `zfr ize`."
            ),
        )
    ]
    ...
    if meson.is_file():
        ...
        for name, body in _run_target_blocks(text):
            if name not in _SCRIPT_TARGETS:
                continue
            if name == "posync" and _command_uses_translate_sync(body):
                continue
            if name == "look" and _command_uses_build_look(body):
                continue
            ...
            if name == "look":
                out.append(
                    Finding(
                        "warn",
                        "layout.scripts",
                        _("run_target('look') should use zfr build --look"),
                        "meson.build",
                        fix=_(
                            "Replace the inline look body with `zfr build --look`. "
                            "Or run `zfr ize`."
                        ),
                    )
                )
