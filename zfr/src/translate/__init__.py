...
def add_arguments(p: argparse.ArgumentParser) -> None:
    p.add_argument(
        "--sync",
        action="store_true",
        help=_("sync gettext catalogs from sources (replaces scripts/posync.sh)"),
    )
    p.add_argument(
        "-C",
        "--chdir",
        metavar="DIR",
        default="",
        help=_("project root for --sync (default: walk from cwd / MESON_SOURCE_ROOT)"),
    )
    p.add_argument(
        "-i",
        "--import",
...
def run(args: argparse.Namespace) -> int:
    ...
    if getattr(args, "sync", False):
        from .sync import sync_catalogs

        sync_root = (
            Path(args.chdir).expanduser().resolve()
            if getattr(args, "chdir", "")
            else None
        )
        return sync_catalogs(sync_root)
