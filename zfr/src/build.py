...
def add_arguments(p: argparse.ArgumentParser) -> None:
    ...
    p.add_argument(
        "--look",
        action="store_true",
        help=_("preview files meson would install (DESTDIR tree; replaces scripts/look.sh)"),
    )
    p.add_argument(
        "-L",
        "--look-level",
        ...
        default=None,
        help=_(
            "with --look, limit tree depth to N levels (implies --look; "
...
def run(args: argparse.Namespace) -> int:
    ...
    look = bool(getattr(args, "look", False) or getattr(args, "look_level", None) is not None)
    if look:
        from build_look import look_install
        ...
        return look_install(
            root,
            builddir=builddir,
            depth=getattr(args, "look_level", None),
        )
