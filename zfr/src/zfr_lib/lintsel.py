...
"""zfr-lintsel — interactive lint rule ignore / always toggles."""
...
from .cmd_options import LINT_OPTIONS_REL
from .i18n import _
from .l10n import lint_options_path, load_lint_option_tokens
...
_SECTION_BEGIN = "# --- zfr-lintsel rules ---"
_SECTION_END = "# --- end zfr-lintsel ---"
...
def run_lintsel_tui(root: Path) -> int:
    if not sys.stdout.isatty() or not sys.stdin.isatty():
        print(_("zfr-lintsel requires an interactive TTY"), file=sys.stderr)
        return 2
    ...
    def _main(stdscr: curses.window) -> int:
        ...
        while True:
            ...
            h, w = stdscr.getmaxyx()
            title = (
                "zfr-lintsel  SPC toggle  Y always  N/- ignore  ~ invert  "
            ...
            try:
                ...
                pass
            meta = (
                f"{root}  {LINT_OPTIONS_REL.as_posix()}  "
            ...
            for row_i in range(visible):
                ...
                ize = "ize" if row.rule.izeable else "   "
                line = f"[{mark}] {row.rule.id}  {ize}  {row.rule.code}  {row.rule.title}"
...
def run(args: argparse.Namespace) -> int:
    return run_lintsel_tui(find_project_dir(getattr(args, "directory", None)))
