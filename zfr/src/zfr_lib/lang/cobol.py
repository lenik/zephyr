...
def _lint(root: Path, role: str) -> list[Finding]:
    if (root / "tests").is_dir():
        return [Finding("ok", "lang.cobol.tests", "tests/ present")]
    return [
        Finding(
            "warn",
            "lang.cobol.tests",
            "no tests/ (cobol template uses cobc + meson test)",
            "tests/",
            fix="Add tests/test_commons.sh or similar smoke test for cobc.",
        )
    ]
