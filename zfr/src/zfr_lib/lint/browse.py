...
def _set_locale(lang: str) -> None:
    import zfr_lib.i18n.messages as messages
...
def _finding_payload(f: Finding, root: Path, rule_states: dict[str, str]) -> dict[str, Any]:
    targets = ize_targets_for_lint(f.code)
    rule = LINT_RULES.lookup(f.code)
    rid = f.rule_id
    docs = rule_doc_dict(rid, f.code)
...
def _collect_payload(
    ...
    *,
    ...
    items = [_finding_payload(f, root, states) for f in findings]
    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def do_GET(self) -> None:  # noqa: N802
            ...
            if parsed.path in ("/", "/index.html"):
                self._send(200, _shell_html().encode("utf-8"), "text/html; charset=utf-8")
                return
            if parsed.path == "/api/data":
                qs = urllib.parse.parse_qs(parsed.query)
                ui_lang = (qs.get("lang") or ["en"])[0]
                with state.lock:
                    _set_locale(ui_lang)
                    payload = _collect_payload(
                        state.root,
                        l10n_level=state.l10n_level,
...
def _resolve_zfr_exe() -> list[str]:
    zfr_src = Path(__file__).resolve().parents[2] / "zfr"
    if zfr_src.is_file():
