...
# Default lint level is L1; see l10n.L10N_LEVELS for L0–L3 tier coverage.
from l10n import L10N_LEVELS, linguas_for_level, parse_l10n_level
from versioning import (  # noqa: F401
    apply_version_modifiers,
    changelog_date,
    changelog_version,
    cli_dependency_versions,
    cli_release_date,
    cli_root,
    cli_version,
    format_cli_version_banner,
...
def pkgdatadir() -> Path:
    ...
    # Checkout: zfr/src/lib.py — templates in monorepo parent.
    # Installed: share/zephyr/zfr/lib.py — pkgdatadir is the parent.
    mod_dir = here.parent
    if (mod_dir / "zfr").is_file():
        if mod_dir.name == "src":
            zfr_pkg = mod_dir.parent
            monorepo = zfr_pkg.parent
            if _looks_like_template_root(monorepo):
                return monorepo
            if _looks_like_template_root(zfr_pkg):
                return zfr_pkg
            return zfr_pkg
...
def _is_zfr_cli_package(root: Path) -> bool:
    ...
    return (root / "src" / "zfr").is_file() and (root / "src" / "lib.py").is_file()
