...
usage() ->
    io:put_chars(tr("Usage: some_puff1 [OPTION]... [FILE]...\n")),
...
main() ->
    ...
    case Verbose of
        true -> io:format(standard_error, "~s: verbose mode enabled~n", ["some_puff1"]);
        false -> ok
    end,
    case Files of
        [] ->
        ...
        _ ->
            case copy_files(Files) of
                ok ->
                    erlang:halt(0);
                {error, {File, Reason}} ->
                    io:format(standard_error, "some_puff1: ~s: ~p~n", [File, Reason]),
                    erlang:halt(1);
                {error, Reason} ->
                    io:format(standard_error, "some_puff1: ~p~n", [Reason]),
                    erlang:halt(1)
            end
    end.
