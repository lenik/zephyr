...
`zephyr` 是一个 Erlang 命令行模板，使用 Meson 做构建/测试/安装。  
`some_puff1` 是此模板中的一个**示例应用**；同一仓库中可以继续添加更多应用。

## 仓库结构

- `src/` - Erlang 源码（`*.erl`）
- `tests/` - Erlang 冒烟/单元测试
- `debian/` - Debian 打包元数据
- `po/` - gettext 翻译目录
...
```bash
sudo apt install meson ninja-build erlang gettext asciidoctor
```
...
Meson 执行 `tests/test_common_lib.erl` 做冒烟/单元测试。
