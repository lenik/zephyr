...
`zephyr` is an OCaml template for small command-line apps, using Meson for build/test/install.
`some_puff1` is one **example app** in this template; more apps can be added in the same repository.

## Repository layout

- `src/` - OCaml source files (`*.ml`)
- `tests/` - OCaml smoke/unit tests
...
Optional: add `dune-project` for Dune-based workflows; Meson compiles with `ocamlfind`/`ocamlc`.
...
Meson runs an OCaml smoke/unit test from `tests/test_commons.ml`.
