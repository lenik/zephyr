...
Version:        %{version}
Release:        1%{?dist}
Summary:        Rust CLI project template with example app
...
BuildRequires:  libgettextpo-dev
BuildRequires:  pkg-config
BuildRequires:  asciidoctor

%description
zephyr is a template repository for small Rust command-line utilities.
It ships the some_puff1 example (Cargo + optional Meson for gettext, docs, and
packaging), and unit tests are run with `cargo test` (via `meson test`).
